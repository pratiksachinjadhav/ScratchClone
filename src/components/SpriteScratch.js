    ...
    return (
        <div className="stage-area overflow-hidden relative bg-white border-2 border-gray-200" style={{ flex: 0.8 }} ref={containerRef} onDragOver={handleDragOver}
            ...
            <div className="absolute top-4 right-3">
                <button
                    onClick={play}
                    className="flex items-center bg-white border-2 text-green-500 border-green-500 rounded-full hover:bg-green-600 hover:text-white font-bold p-2 justify-center items-center transition duration-300"
                >
                    <FlagIcon className=" font-bold  hover:text-white" size={20} />
                </button>
            </div>
        </div>
    );
